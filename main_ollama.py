...
class LatestSummaryStore:
    ...
    def set(self, summary: dict[str, Any]) -> None:
        with self._lock:
            self._summary = json.loads(json.dumps(summary))
...
class OllamaConsoleAgent:
    def __init__(self, summary_store: LatestSummaryStore, model: str, enabled: bool) -> None:
    ...
    def _run(self) -> None:
        while not self._stop_event.is_set():
            ...
            summary = _summary_to_status_payload(self._summary_store.get())
            prompt = self._build_prompt(question, summary)
            response = self._query_ollama(prompt)
            if response:
                print(response)

    def _build_prompt(self, question: str, status_json: dict[str, Any]) -> str:
        status_text = json.dumps(status_json, ensure_ascii=False)
        return (
            "You are assisting with a radar presence monitoring console.\n"
            "The distance is in meters, the meter is up to 4m, not more.\n"
            "Use the radar status JSON to answer the user's question.\n"
            "If the radar data is uncertain, say so explicitly.\n"
            "Keep the answer concise and directly useful.\n\n"
            "The energy is just number not dbm.\n"
            f"Radar status JSON:\n{status_text}\n\n"
            f"User question:\n{question}\n"
        )
...
def _run_recorded_mode(
    args: argparse.Namespace,
    rt: dict[str, Any],
    summary_store: LatestSummaryStore,
) -> dict[str, Any]:
    a2 = rt["a2"]
    ...
    if tracking_processor is not None:
        ...
        try:
            for frame_idx, result in enumerate(_iter_record_results(record), start=1):
                ...
                summary_store.set(final_summary)
...
def main() -> int:
    ...
    summary_store = LatestSummaryStore()
    summary_store.set({"presence": False, "targets": []})
    ...
    try:
        if args.input_file:
            _run_recorded_mode(args, rt, summary_store)
        else:
            _run_live_mode(args, rt, summary_store)
    except RuntimeError as exc:
        print(str(exc), file=sys.stderr)
        console_agent.stop()
        return 2
