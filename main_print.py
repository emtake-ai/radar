...
class OpenCvHeatmapViewer(HeatmapViewer):
    _HEATMAP_WIDTH_SCALE = 8
    _HEATMAP_HEIGHT_SCALE = 12
    ...
    def update_result(
        ...
        cv2 = self._cv2
        try:
            raw_frame = self._heatmap_to_frame(raw_heatmap, np, cv2, "Raw Heatmap")
            suppressed_frame = self._heatmap_to_frame(
                suppressed_heatmap, np, cv2, "Suppressed Heatmap"
            )
            if raw_frame is None or suppressed_frame is None:
                return
            ...
            frame = cv2.hconcat(
                [
                    raw_frame,
                    suppressed_frame,
                    self._build_text_panel(raw_frame.shape[0], summary, cv2),
                ]
            )
    ...
    def _heatmap_to_frame(self, heatmap: Any, np: Any, cv2: Any, title: str) -> Any:
        heatmap_2d = _heatmap_to_angle_view(heatmap, np)
        if heatmap_2d is None:
            return None

        frame = cv2.normalize(heatmap_2d, None, 0, 255, cv2.NORM_MINMAX)
        frame = frame.astype("uint8")
        frame = cv2.applyColorMap(frame, cv2.COLORMAP_JET)
        frame = cv2.resize(
            frame,
            None,
            fx=self._HEATMAP_WIDTH_SCALE,
            fy=self._HEATMAP_HEIGHT_SCALE,
    ...
    def _build_text_panel(self, height: int, summary: dict[str, Any], cv2: Any) -> Any:
        ...
        payload = _summary_to_status_payload(summary)
        lines = [
            f"time: {payload['time']}",
            f"presence: {str(payload['presence']).lower()}",
            f"distance: {payload['distance']}",
            f"azimuth: {payload['angle']['azimuth']}",
            f"elevation: {payload['angle']['elevation']}",
...
class TargetStabilizer:
    def __init__(
        self,
        distance_smoothing_alpha: float = 0.25,
        angle_smoothing_alpha: float = 0.25,
        energy_threshold: float = 20.0,
        max_distance_jump_m: float = 0.75,
        max_angle_jump_deg: float = 15.0,
        switch_confirm_frames: int = 3,
        presence_on_frames: int = 2,
        presence_off_frames: int = 4,
        target_hold_frames: int = 16,
    ) -> None:
    ...
    def _select_candidate(self, candidates: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        strongest = max(candidates, key=lambda candidate: float(candidate["energy"]))
        if self._stable_target is None:
            self._pending_target = None
            self._pending_count = 0
            return strongest

        nearest = min(candidates, key=lambda candidate: self._candidate_cost(candidate, self._stable_target))
        dist_jump = abs(float(nearest["distance_m"]) - float(self._stable_target["distance_m"]))
        az_jump = abs(
            float(nearest["angle_deg"]["horizontal"]) - float(self._stable_target["angle_deg"]["horizontal"])
        )
        el_prev = self._stable_target["angle_deg"]["elevation"]
        el_curr = nearest["angle_deg"]["elevation"]
        el_jump = 0.0 if el_prev is None or el_curr is None else abs(float(el_curr) - float(el_prev))

        if dist_jump <= self.max_distance_jump_m and az_jump <= self.max_angle_jump_deg and el_jump <= self.max_angle_jump_deg:
            self._pending_target = None
            self._pending_count = 0
            return nearest

        if self._pending_target is not None and self._candidate_cost(strongest, self._pending_target) < 0.35:
            self._pending_count += 1
        else:
            self._pending_target = strongest
            self._pending_count = 1
...
def _summary_to_status_payload(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "time": datetime.now().strftime("%H:%M:%S"),
        "presence": bool(summary.get("presence", False)),
        "distance": round(float(target["distance_m"]), 2),
        "angle": {
            "azimuth": round(float(target["angle_deg"]["horizontal"]), 2),
            "elevation": (
        ...
        "energy": round(float(target["energy"]), 2),
    }
...
class StatusPrinter:
    def __init__(self, period_s: float = 1.0) -> None:
        ...
        self._last_print_monotonic: float | None = None

    def maybe_print(self, summary: dict[str, Any]) -> None:
        now = time.monotonic()
        if self._last_print_monotonic is None or now - self._last_print_monotonic >= self._period_s:
            print(json.dumps(_summary_to_status_payload(summary), ensure_ascii=False))
...
def _run_recorded_mode(args: argparse.Namespace, rt: dict[str, Any]) -> dict[str, Any]:
    ...
    status_printer = StatusPrinter(period_s=1.0)
    ...
    if tracking_processor is not None:
        ...
        try:
            for frame_idx, result in enumerate(_iter_record_results(record), start=1):
                ...
                status_printer.maybe_print(final_summary)
...
def main() -> int:
    args = _parse_args()
    try:
        rt = _import_runtime(enable_visualization=args.visualization)
